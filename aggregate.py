...
def video_label_length(dataset='shanghaitech'):
    label_path = './frame_label/' + dataset + '_label'
    label_files = sorted(os.listdir(label_path))
    video_length = {}
    if dataset in ['ped2', 'avenue', 'shanghaitech']:
        for label_name in label_files:
            label = np.load("{}/{}".format(label_path, label_name))
            label_len = label.shape[0]
            video_length[label_name.split(".")[0]] = label_len
    else:
...
# 原ws=43
def score_smoothing(score, ws=23, function='mean', sigma=10):
...
def load_objects(dataset_txt, dataset, frame_num=7):
    root = 'F:\Jigsaw'
    data_dir = os.path.join(root, dataset)
    # file_list = sorted(os.listdir(data_dir))

    with open(file=dataset_txt, mode='r') as f2:
        file_list = f2.readlines()
    file_list.sort()

    detect_dir = f'./UVAD_detect/{dataset}_detect.pkl'
    ...
    for video_file in file_list:
        ...
        if 'train' in video_file:
            l = os.listdir(data_dir + '/training/frames/' + video_file)
            start_ind = frame_num - 1
        elif len(video_file) == 2:
            l = os.listdir(data_dir + '/testing/frames/' + video_file)
        ...
        for frame in range(start_ind, length - start_ind):
            ...
            for i in range(object_num):
                if not is_contain[i]:
                    if not is_small[i]:
                        objects_list.append(
                            {"video_name": video_file, "frame": frame, "object": i, "loc": detect_result[i, :4]})
                    else:
                        total_small_ += 1
                else:
                    contain += 1
...
def remake_video_3d_output(dataset_txt, video_output, dataset='ped2', frame_num=7):
    '''
    ...
    :return:
    '''
    object_list = load_objects(dataset_txt, dataset, frame_num=frame_num)
...
def remake_video_output(video_output, dataset='shanghaitech'):
    video_length = video_label_length(dataset=dataset)
    return_output_spatial = []
    return_output_temporal = []
    return_output_complete = []
    ...
    for i in range(len(video_l)):
        ...
        if len(video) == 7:
            ...
            for fno in frame_l:
                object_record = frame_record[fno]
                object_record = np.array(object_record)
                video_[count], video2_[count] = object_record.min(0)

                local_max_ = max(object_record[:, 0].max(), local_max_)
                local_max2_ = max(object_record[:, 1].max(), local_max2_)
                count += 1
        else:
            raise ValueError("video {} error!".format(video))
        ...
        combined_video = 0.5 * video2_ + 0.5 * video_
...
# video_output
def evaluate_auc(logger, video_output, dataset_txt, dataset='shanghaitech'):
    result_dict = {'dataset': dataset, 'psnr': video_output}
    smoothed_results, aver_smoothed_result = evaluate.evaluate_all(result_dict, dataset_txt, reverse=True,
                                                                   smoothing=True)
    # print("(smoothing: True): {}  aver_result: {}".format(smoothed_results, aver_smoothed_result))
    logger.info("(smoothing: True): {}  aver_result: {}".format(smoothed_results, aver_smoothed_result))
...
if __name__ == '__main__':
    ...
    if args.dataset == 'shanghaitech':
        video_output_spatial, video_output_temporal, video_output_complete = remake_video_output(output,
                                                                                                 dataset=args.dataset)
    else:
        video_output_spatial, video_output_temporal, video_output_complete = remake_video_3d_output(output,
                                                                                                    dataset=args.dataset,
