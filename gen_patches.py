...
class VideoAnomalyDataset(Dataset):
    ...
    def save_objects(self):
        ...
        for i in tqdm(range(len(self.objects_list))):
            ...
            video_dir = os.path.join(self.save_root, self.dataset, self.phase, record["video_name"])
    ...
    def read_single_frame(self, video_dir, frame, frame_list):
        ...
        assert (frame_ in frame_list),\
            "The frame {} is out of the range:{}.".format(int(frame), len(frame_list))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--phase", type=str, default='testing', choices=['training', 'testing'])
    parser.add_argument("--phase2", type=str, default='test', choices=['train', 'test'])
    ...
    save_root = 'your save_path'
    shanghai_dataset = VideoAnomalyDataset(# data_dir=data_dir + args.dataset + '/' + args.phase + 'ing/',
                                           data_dir=data_dir + args.dataset + '/' + args.phase + '/frames/',
                                           detect_dir='./UVAD_detect/{args.dataset}_detect.pkl',
